...
let rec show_expr = function
  ...
  | Length e -> sprintf "length(%s)" (show_expr e)
  | Unit e -> sprintf "unit(%s)" (show_expr e)
...
let show_data_def (dfn : data_def) : string =
  let show_sels pairs =
    if List.length pairs > 0 then
      ...
      ^ String.concat ", "
          (List.map
             (fun (x, s) -> sprintf "%s : %s" x.value (show_sort s))
             pairs )
      ^ "}"
    else ""
  in
  ...
  in
  sprintf "data%s %s := %s" params dfn.name.value
    (String.concat " | "
       (List.map
          (fun c -> sprintf "%s%s" (fst c).value (show_sels (snd c)))
          dfn.cases ) )
